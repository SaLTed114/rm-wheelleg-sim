...
本文记录 ACTIVE 内部的职责划分、数据流和状态机边界。它是下一轮重构和离地控制接入的架构约束，不代表所有模块已经实现。当前代码只实现了 NORMAL 下的纵向 `HOLD/VELOCITY`、云台跟随、front/rear 映射和工作腿长规划；这些逻辑仍集中在 `bc_motion_t` 与 `bc_motion_action()` 中。SPIN、support phase 和独立的 ACTIVE 协调器尚未实现。
...
当前平台实验只验证了 `0.18 m -> 0.38 m` 的空中快速伸腿和腿角 LQR；触地顺应、吸能、回收速率及 `400 mm` 工况尚未定稿。因此这些只能作为 support policy 的候选，不应提前固化为通用 ACTIVE 参数。
...
该结构属于 ACTIVE 内部，但阶段和转换条件尚未进入生产状态机。目前可以确认的边界是：

- 生产 `control/support_force` 已输出每侧带迟滞的 `GROUND/AIR` 诊断，但尚未进入 state-machine input；
- 平台 benchmark 仍用 MuJoCo 接触真值切换阶段，并通过 `step_with_control_transform()` 临时修改正式状态机生成的控制命令；这只是实验接管器，必须在生产 support phase 建立后移除；
