...
正式 controller case 已验证 `0.18 m -> 0.38 m` 的空中快速伸腿、腿角 LQR、触地轴向阻抗、快速收腿和无扰恢复，并覆盖正反向 `200/400 mm` 平台驶离。参数仍只有理想仿真依据，实车噪声、结构柔性和不平地面尚未验证，因此它是首版仿真策略而非实车定稿。
...
生产接管验证完成后，benchmark 中复制完整落地流程的 C++ 悬挂控制器、五组 `K/D` 扫描和对应单元测试已退役。平台工具只保留 `200 mm、2.0 m/s、0.18 -> 0.38 m` 的两个空中 `length_only/leg_lqr` 机理对照，以及正反向 `200/400 mm` 四个生产 ACTIVE 落地案例；后者全程调用普通控制入口，不再由仿真层改写腿力或回收参考。完整探索过程和删除依据见 `docs/notes/platform-drop-exploration-archive.md`。
...
离地融合也不能因平台案例中的小延迟而视为完成。当前支持力与 IMU 比力组合尚未系统区分真实双轮离地、斜坡过渡、平台边缘逐轮卸载、颠簸和强纵向动态；后续场地实验必须同时检查误报、漏报、左右轮时序和阶段抖动，再决定是否调整阈值、持续时间或增加新的观测量。
