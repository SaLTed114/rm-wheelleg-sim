...
4. 继续检查融合闭环中的正向偏心平移和停转 yaw 残速，不再恢复直接轮速 `S/DS` 路径。
5. 将当前 `20 ms` 可信度迟滞、IMU 外参和噪声参数视为仿真初值；实车数据到位后重新标定，同时保留 `A/B + K` 作为 trim 附近的初期先验，不用它替代高偏航率非线性验证。
...
8. 继续把原始轮角、轮速、prior/posterior、可信度和 NIS 保留为独立诊断量，避免正式状态收口后失去问题定位依据。
