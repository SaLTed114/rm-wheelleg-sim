...
- 此前为直线因果对照临时加入的 `S/DS` feedback config 已移除，避免把实验注入伪装成生产配置；相关结果仍有效，但新实验需等待 ACTIVE 下的显式运动模式通过禁用位图表达同一策略；
...
- 直线 `A/B/K` 对照由 `tools/lqr/forward_response.py` 围绕运动前最后 `1 s` 稳态运行，并在预测中显式令 `S` 误差为零。默认 `0.18 m`、关闭 `S`、站立 `8 s` 后的 `+2 m/s` 正向 `0.5/1/5 m/s^2` 案例均全程双轮接触且无饱和；运动 ramp/hold 内 MuJoCo/线性模型 pitch 峰值分别约为 `1.78/0.64`、`1.77/0.71`、`1.81/1.80 deg`，pitch RMS 模型误差约为 `0.71/0.74/0.83 deg`，`DS` RMS 模型误差约为 `0.049/0.054/0.068 m/s`；停止 ramp/settle 内 pitch 峰值分别约为 `2.18/1.34`、`2.55/1.99`、`3.77/3.07 deg`，pitch RMS 模型误差约为 `0.66/0.66/0.61 deg`。因此 `5 m/s^2` 的大俯仰既有当前 `K + DS` 制动参考自身的贡献，也有约 `0.7 deg` 的 plant/非线性偏差，不能单独归因于任一方；
...
3. 继续隔离直线 `S` 位置反馈造成的 pitch 慢恢复与停止残摆，联合参考轨迹整形和控制参数做可复现对照，不用瞬时跟踪 RMSE 主导选择。
