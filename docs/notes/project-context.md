...
- 搬运腿上台阶不是步态或自动地形识别，而是操作手触发的 task：先建立约 `0.38 m` 准备姿态和至少 `200 mm` 底盘净空，以 `2.0 m/s`、`3.0 m/s^2` 的受限包线正对台阶前进，碰撞后执行固定搬腿动作并重新平衡。隔离 `step_dock` 证明立即切断控制后机体仍能搭住 `200 mm` 台面；历史 `0/5/10/20/30 ms` 延迟扫描只用于确认最多等待约 `10 ms` 的姿态代价很小，常驻 GUI case 已收缩为立即切断和 `10 ms` 两例。电机力反算不含接触约束外力，旧水平力残差不能升级为碰撞判据。
...
- 加速未完成时撞台阶也属于正样本。CTest 现按底盘从加速起点实际行驶 `0.3/0.6/0.9/1.2/1.5 m` 校准平台位置和启动 yaw，实际距离误差不超过约 `4.8 mm`，碰撞速度依次约为 `1.20/1.61/1.78/1.87/1.92 m/s`。五组都在速度稳定门禁前撞击，最终都留在平台上且末 `250 ms` 顶面接触率为 `100%`，但仍有残余摆动，不能称为被动完全停稳。`5 ms` 腿角速度增量和轮速-IMU 失配最弱仍约为 `1.66 rad/s`、`0.447 m/s`，全部非碰撞包线最大仅约 `0.110 rad/s`、`0.030 m/s`；前向 Delta-v 在加速背景下更不能解释为减速证据。状态机首版可探索两项在同一 `5 ms` 窗口分别超过 `0.5 rad/s`、`0.12 m/s` 并持续 `2 ms` 的融合候选，不要求助跑距离或速度稳定，只在 task 准备姿态内暂留约 `0.3 m/s` 的低速防误触门槛。候选尚未进入控制器，也不是生产阈值；低速顶墙、颠簸、结构柔性、实车噪声及门禁语义仍需验证。
