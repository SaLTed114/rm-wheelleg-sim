...
- motion config、performance benchmark 和外部实验 runner 已增加默认开启的 `yaw_position_feedback` 独立开关；关闭时只令 `ref_psi=psi`，`DPSI` ramp 保持不变。使用 `1200/240` 与正式差分权重 `1920/32` 时，关闭 `PSI` 已使原本摔倒的正负 `4*pi / a7.5` 均完整跟踪并在 `5 s` 内停稳，速度峰值约 `12.23/12.22 rad/s`，不再超调；但 `a10/a15` 仍失败。进一步联合 `7680/128` 强差分权重后，正负 `a10` 也完整通过并停稳，峰值约 `12.25/12.23 rad/s`、RMSE 约 `0.322/0.344 rad/s`；`a15` 正向仍在 ramp 摔倒，负向虽满足速度跟踪但在 stop settle 非轮触地，均不安全。因此之前的加速度边界确实由 `PSI` 追角度和腿差分两个控制问题叠加；当前 `a15` 剩余瓶颈才主要是执行器/接触极限。该开关目前只用于隔离实验，正式方案仍应在运动时关闭 `PSI`、低速稳定后捕获当前角度并重新开启驻车锁定，而不是全程永久关闭；
