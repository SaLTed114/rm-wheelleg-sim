...
- 当前阶段聚焦 plant 的坐标、几何和物理参数，并准备新的建模任务。除非明确要求，不要主动恢复控制器扩展、运动性能调参或“为什么实车没调好”的诊断。
...
- 暂停继续扩展控制器和运动性能，回到机器人 plant 本身开始新的建模工作；
- 先建立可追溯的坐标系和物理参数链：确认每个刚体的期望原点/坐标轴、网格变换、质量、质心和质心处惯量，再决定如何替换当前上游参数；
- 新模型必须保留实际多连杆闭链、偏置质心和惯量分布，不为适配现有 LQR 而把 plant 对称化或等效成虚拟腿；
- 当前控制器、补偿、LQR 调度和性能基线都只对修改前的 plant 有效。模型参数或坐标定义改变后，必须重新提取等效参数、生成增益并运行验证，不能用旧闭环表现反向约束新模型；
- 具体建模任务由后续讨论逐项确定。在目标尺寸和物理量没有可靠来源前，不根据现有仿真表现猜测并修改参数。
...
- 当前虚拟髋轴位于机体局部 `z=-0.05 m`，`base_link.STL` 最低点约为 `z=-0.226922 m`，髋轴到机体最低点约 `176.9 mm`；按 `0.18 m` 腿长和约 `58.06 mm` 轮半径估算，机体最低点理论离地仅约 `61.1 mm`。当前复杂机体 mesh 直接参与地面碰撞，尚无独立简化碰撞体；这组几何是新建模阶段需要优先复核的对象。
...
2. 逐个确定新模型 body frame，并核对 CAD/STL 到 body 的显式变换；优先复核机体底部、髋轴和轮轴之间的实际几何关系。
3. 为每个刚体建立有来源的质量、质心和质心处惯量，不用旧 MATLAB 参数或现有闭环表现填补未知量；当前单侧腿轮 `4 kg` 和机体大惯量均需要复核。
4. 确定外观 mesh 与简化碰撞几何的职责，避免复杂 STL 碰撞细节掩盖真实控制或接触问题。
5. 新 plant 定稿后重新验证闭链、关节符号、运动学/Jacobian 和自由落体，再重新提取等效腿参数、生成 LQR 调度并标定工作点补偿。
6. 此后再恢复 `S` 通道静态复测、直线/yaw suite、正向偏心平移和停转残速调查；不把修改前 plant 的阈值直接沿用为新基线。
7. 将当前 `20 ms` 可信度迟滞、IMU 外参和噪声参数视为仿真初值；实车数据到位后重新标定，且不恢复直接轮速 `S/DS` 路径。
