...
4. 基于已定位的轮速污染，设计正式的接触可信度、IMU/编码器融合或自旋工况速度观测策略；不要直接把 benchmark 的 MuJoCo 真值替换或固定 `50 ms` 冻结搬进控制核心。
5. 用 `yaw-acceleration` suite 验证正式 observer，同时保留 `A/B + K` 作为 trim 附近的初期先验，不用它替代高偏航率非线性验证。
