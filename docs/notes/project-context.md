...
4. 在融合观测器接管正式 `S/DS` 前，继续区分 yaw 中的短时轮速跳变与持续滑移，明确大量 NIS 拒绝后的恢复/可信度策略；不要直接使用 MuJoCo 接触真值或固定 `50 ms` 冻结。
5. 用完整 `yaw-acceleration` suite 扫描 estimator 的方向与加速度边界，同时保留 `A/B + K` 作为 trim 附近的初期先验，不用它替代高偏航率非线性验证。
...
8. 融合速度接管后让 `S` 对融合后的 `DS` 积分，并继续把原始轮角、轮速、prior/posterior 和 NIS 保留为独立诊断量。
