...
int main() {
    ...
    if (fabsf(command.state_reference.value[BC_STATE_S] - 1.02F) > 1.0e-6F ||
        fabsf(command.state_reference.value[BC_STATE_DS] - 0.2F) > 1.0e-6F ||
        fabsf(command.state_reference.value[BC_STATE_PSI] + 0.53F) > 1.0e-6F ||
        fabsf(command.state_reference.value[BC_STATE_DPSI] + 0.3F) > 1.0e-6F) {
        fputs("balance reference was assembled incorrectly\n", stderr);
        return 1;
    }
    for (int index = BC_STATE_THETA_L; index < BC_STATE_NUM; ++index) {
